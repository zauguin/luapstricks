...
\def\ps@readmargin#1 #2 #3 #4 #5\@nil{%
    ...
    \ifx\\#3#4\\%
        \def\ps@margin@left{#1 }%
        \let\ps@margin@right\ps@margin@left
        \def\ps@margin@bottom{#2 }%
        \let\ps@margin@top\ps@margin@bottom
    \else
        \def\ps@margin@left{#1 }%
        \def\ps@margin@bottom{#2 }%
        \def\ps@margin@right{#3 }%
        \def\ps@margin@top{#4 }%
    \fi\fi
\typeout{>>>>>>>>>\ps@margin@left,\ps@margin@bottom,\ps@margin@right,\ps@margin@top}%
}%
\define@key[psset]{pstricks}{margin}[0.5]{\ps@readmargin#1 {} {} {} {} \@nil}
\psset[pstricks]{margin=0.5}
%
\newcount\psimagectr
\psimagectr=0
%
...
\def\pst@@picture@@i[#1]{%
  \def\pscoor{(10,10)}%
  \typeout{>>>Check for file \jobname-\the\numexpr\psimagectr+1.psaux}%
  \IfFileExists{\jobname-\the\numexpr\psimagectr+1.psaux}{\input{\jobname-\the\numexpr\psimagectr+1.psaux}}{}%
  \typeout{>>>\pscoor}%
  \edef\@temp{[#1]\pscoor}%
...
% ----------- 1.10/12 end hv -------------------
  ...
  \pstVerb{ .trackbbox }\global\advance\psimagectr by 1
...
%     XMIN YMIN XMAX XMIN sub YMAX YMIN sub
...
%     gsave currentpoint translate newpath rectstroke grestore %  nur für den test
     (\jobname-\the\psimagectr.psaux) (w) file /CoorFile exch def
     (\\gdef\\pscoor{\string\() CoorFile exch writestring
     XMIN \ps@margin@left sub 20 string cvs CoorFile exch writestring
     (pt, ) CoorFile exch writestring
     YMIN \ps@margin@bottom sub 20 string cvs CoorFile exch writestring
     (pt\string\)\string\() CoorFile exch writestring
     XMAX \ps@margin@right add 20 string cvs CoorFile exch writestring
     (pt, ) CoorFile exch writestring
     YMAX \ps@margin@top add 20 string cvs CoorFile exch writestring
