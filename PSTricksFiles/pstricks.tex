...
\ifPSTlualatex
  ...
  \def\pstVerb{\luaPST direct}%
  \def\pstVerbset{\luaPSTheader inline}%
  \def\pstunit{1bp}%
  \def\pstverbscale{}%
  \def\pstnodescale{}%
...
\def\pst@Verbset#1{\pstVerbset{\pst@dict #1 end}}
\def\tx@Atan{Atan }
\def\tx@Div{Div }
\def\tx@NET{NET }
\def\tx@Pyth{Pyth }
...
\def\@degrees[#1]{%
  ...
  \pstVerbset{tx@Dict begin startGlobal /pst@angleunit {\pst@angleunit} def endGlobal end}%
  \ignorespaces}
%
...
\def\psset@@rot#1\@nil{%
  \def\next##1@#1=##2@##3\@nil{%
    ...
    \pst@Verbset{ gsave  STV CP T /ps@rot \ifx\psk@rot\@empty 0 \else \psk@rot \fi def grestore }% (MJS)
  }%
...
\def\psset@@refangle#1\@nil{%
  ...
  \pst@Verbset{ gsave STV CP T /ps@refangle \psk@refangle\space def grestore }%ADDED (MJS)
}
%
...
\pstVerbset{0.8 setlinewidth 0 setgray}%    default setting (needed for lualatex)
